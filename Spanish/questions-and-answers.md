...
2. [Otros tipos de solicitudes en un product backlog](#2-qué-otros-tipos-de-solicitudes-se-pueden-incorporar-en-un-product-backlog-además-de-requerimientos-del-cliente)
3. [Criterio para ordenar un product backlog](#3-qué-criterio-optaría-para-ordenar-un-product-backlog)
4. [Cantidad de product backlogs por proyecto](#4-cuántos-product-backlogs-podemos-tener-por-proyecto)
5. [Participación obligatoria del PO en ceremonias Scrum](#5-en-qué-ceremonias-de-scrum-debe-participar-obligatoriamente-el-product-owner)
6. [Error crítico de QA con sprint en curso](#6-si-el-equipo-de-qa-encuentra-un-error-crítico-con-el-sprint-en-curso-qué-decisiones-tomarías)
7. [Incidente productivo urgente en mitad de un sprint](#7-incidente-productivo-urgente-reportado-por-soporte-en-mitad-de-un-sprint-cómo-lo-abordarías)
8. [Producto atrasado en su desarrollo](#8-el-producto-está-atrasado-en-su-desarrollo-qué-pasos-tomarías-para-minimizar-el-impacto)
9. [Cambio urgente del cliente en mitad de un sprint](#9-un-cliente-solicita-un-cambio-urgente-en-mitad-de-un-sprint-cómo-lo-abordarías)

---

## 1. ¿Cuál es la finalidad de los criterios de aceptación?

Para mí, la finalidad principal es eliminar la ambigüedad. Cuando escribís una historia de usuario sin criterios de aceptación, cada persona del equipo puede interpretar "listo" de forma distinta. El desarrollador cree que terminó, QA cree que falta la mitad, y el cliente esperaba algo diferente.

Los criterios de aceptación establecen de antemano qué tiene que pasar exactamente para que todos estén de acuerdo en que la historia está completa. Desde mi experiencia en QA, son también la base directa de los casos de prueba — si un criterio está bien escrito en formato Dado/Cuando/Entonces, casi se convierte en un test automatizado por sí solo.

---

## 2. ¿Qué otros tipos de solicitudes se pueden incorporar en un product backlog además de requerimientos del cliente?
...
- **Refinement / Backlog Grooming** — aunque técnicamente no es una ceremonia oficial de Scrum, en la práctica es donde el PO trabaja más: prioriza, clarifica y descompone historias junto al equipo
...
Con mi experiencia en QA, lo primero es evaluar el impacto real: ¿afecta funcionalidad ya entregada en producción, o es algo dentro del sprint actual? ¿Hay workaround posible?

Si es crítico y bloquea la historia en curso, lo primero es comunicarlo de inmediato al equipo — no esperar a la daily del día siguiente. En la práctica esto implica levantar el tema directamente con el desarrollador responsable para que lo tome lo antes posible.

Si el bug afecta el compromiso del sprint, hay que hablarlo con el Scrum Master y el PO para evaluar si se ajusta el alcance del sprint, se prioriza el fix sobre otras tareas, o se escala si el impacto supera lo que el equipo puede absorber. Como PM o PO, nunca ignoraría un crítico solo para proteger el sprint — eso acumula deuda y riesgo real.

---

## 7. Incidente productivo urgente reportado por soporte en mitad de un sprint, ¿cómo lo abordarías?

Lo primero es entender el impacto: ¿cuántos usuarios afecta, qué funcionalidad está caída, hay pérdida de datos involucrada?
...
Con mi background en QA, también me interesaría entender si el incidente pasó por testing y cómo — esa conversación siempre aporta al proceso.

---

## 8. El producto está atrasado en su desarrollo, ¿qué pasos tomarías para minimizar el impacto?

Primero entender por qué está atrasado — no es lo mismo deuda técnica acumulada que scope creep que estimaciones incorrectas. Cada causa tiene una respuesta distinta.
